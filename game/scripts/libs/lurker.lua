...
function lurker.listdir(path, recursive, skipdotfiles)
  ...
  for _, f in pairs(lume.map(dir(path), fullpath)) do
    if not skipdotfiles or not f:match("/%.[^/]*$") then
      if not lume.find(lurker.ignores, f) then
...
function lurker.getchanged()
  local function fn(f)
    return lurker.files[f] ~= lastmodified(f).modtime
  end
...
function lurker.hotswapfile(f)
  ...
  if f:sub(#f - 2, #f) == "png" then
    local ogf = f
    f = f:sub(#"assets/images/" + 1, #f - 4)
    Asset.image(f, true)
    print("Swapped image " .. f)
    lurker.resetfile(ogf)
    lurker.postswap(ogf)
    if lurker.protected then
      lurker.updatewrappers()
    end
  ...
  if f:sub(#f - 3, #f) == "ldtk" then
    local ogf = f
    f = f:sub(#"assets/maps/" + 1, #f - 5)
    Asset.map(f, true)
    print("Swapped map " .. f)
    lurker.resetfile(ogf)
    lurker.postswap(ogf)
