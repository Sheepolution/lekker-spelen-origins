...
function set:__index(n)
	local f = rawget(set, n)
	if f then
		return f
	end
	...
	if n:find("_") and n:sub(#n, #n) == "_" then
		local k = f:sub(0, #f - 1)
		self[n] = function(s, ...)
			for i = #s._ordered, 1, -1 do
				if s._ordered[i][k] then
					s._ordered[i][k](s._ordered[i], ...)
				end
			end
		end
	else
		self[n] = function(s, ...)
			for i = 1, #s._ordered do
				if s._ordered[i][n] then
					s._ordered[i][n](s._ordered[i], ...)
				end
			end
		end
	end
